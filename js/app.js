...
import {
  getTodaysFixtures, getTodayAndTomorrowFixtures, getFeaturedFixtures,
  getMockPredictions, buildOraclePrediction, getAPIPrediction, POPULAR_LEAGUES
} from './sportsApi.js';
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Auth state — persists across page reloads via Firebase's local persistence
  onAuthChange(async (user) => {
    ...
    if (user) {
      try {
        ...
        if (!userProfile) {
          // Profile doc missing (e.g. first-time Google sign-in) — create it now
          const { createUserProfile } = await import('./firebase.js');
        ...
        showToast('Could not load your profile. Check Firestore rules.', 'error');
        // Still show a basic logged-in state so the user knows they're authenticated
        renderUserBar({ displayName: user.displayName || user.email, xp: 0, coins: 0 });
      }
    } else {
      currentUser = null;
      userProfile = null;
...
async function loadPredictions() {
  ...
  try {
    ...
    const pool = fixtures.slice(0, 4);

    if (pool.length) {
      const builtPreds = await Promise.all(
        pool.map(f => buildOraclePrediction(f.fixture.id).catch(() => null))
      );
      predictions = builtPreds
        .map((p, idx) => p ? { ...p, matchDate: pool[idx].fixture?.date } : null)
        .filter(Boolean);
    }
  } catch (e) {
  ...
  return `
    <div class="glass-card prediction-card reveal reveal-delay-${(i%4)+1}" 
      ...
      <div class="pred-header">
        ...
        <span class="pred-time">${formatMatchTime(p.matchDate, p.kickoff)}</span>
      </div>
...
function formatMatchTime(dateStr, fallback) {
  if (!dateStr) return fallback || '—';
  const date = new Date(dateStr);
  const now = new Date();
  const isTomorrow = date.toDateString() !== now.toDateString();
  const time = date.toLocaleTimeString('en-NG', { hour: '2-digit', minute: '2-digit' });
  return `${isTomorrow ? 'Tomorrow' : 'Today'} · ${time}`;
}
...
// ── Match Center ──────────────────────────────────────
async function loadMatchCenter(leagueId = 1) {
  const list = document.getElementById('matches-list');
  if (!list) return;
  list.innerHTML = `<div class="loading-overlay"><div class="spinner"></div></div>`;
  try {
    const fixtures = await getTodayAndTomorrowFixtures(leagueId);
    if (!fixtures.length) throw new Error('No fixtures');
    list.innerHTML = fixtures.slice(0, 10).map(renderMatchItem).join('');
  } catch {
    list.innerHTML = `<p style="color:var(--text-muted);text-align:center;padding:30px">No fixtures found for today or tomorrow in this league. Try another league.</p>`;
  }
}
...
function renderMatchItem(f) {
  ...
  const timeLabel = formatMatchTime(f.fixture?.date, status);
  return `
    <div class="glass-card match-item" onclick="loadMatchDetail(${f.fixture.id})">
      ...
      <div class="match-teams">
        ...
        <div class="match-team-row">
          <span>${away}</span>
          <span style="font-size:0.68rem;color:${statusColor}">${timeLabel}</span>
        </div>
      </div>
...
window.switchAuthTab = function(tab, el) {
  ...
  const sub     = document.querySelector('#auth-modal .modal-sub');
  if (nameRow) nameRow.style.display = tab === 'register' ? 'block' : 'none';
  if (btn)     btn.textContent = tab === 'register' ? 'Create Account' : 'Login';
  if (title)   title.textContent = tab === 'register' ? 'Create Account' : 'Welcome Back';
  if (sub)     sub.textContent = tab === 'register'
    ? 'Join OracleSports.ai and start earning XP today.'
    : 'Access your AI predictions, challenges, and XP dashboard.';
};
